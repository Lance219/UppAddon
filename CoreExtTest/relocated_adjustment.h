struct VectorConstructNValueD{
	...
	VectorConstructNValueD(const VectorConstructNValueD& d): d(d.d){ if( ++i == 11 ) throw 5; }
...
class RelocAdjustSimple{
	struct Node{
		...
		char * UseBuff(){ return buff; }
	};
	
	Node * node;
	char buff[1024];
public:
	RelocAdjustSimple()noexcept: node(nullptr){}
	...
	~RelocAdjustSimple()noexcept{ delete node;/* LOG("RelocAdjustSimple Object destructed");*/}
	
	void AddNode(){ if(!node) node = new Node{this};}
	
	void DoPostRelocationAdjustment()noexcept{
		if(node) node->owner = this;
	}
	
	bool IsConsistent()const{ return node->owner == this; }
