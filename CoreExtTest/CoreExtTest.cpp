...
TEST(CoreExtTest, VectorInsertMoveExceptionSafety)
{
	...
	struct S{
		S(int i){
			p = nullptr;
			if( i == m ) throw std::runtime_error("fictitious");
			p = new int(i);
		}
		//S(const S& s){ p = nullptr; if(s.p) p = new int(s.Get());}
		S(S&& s){ p = s.p; s.p = nullptr; }
		
		~S(){ delete p; }
		void DoPostRelocationAdjustment()noexcept{}
		
		int Get()const{ if(!p) throw std::runtime_error("uninitialized S"); return *p;}
		int * p;
	};
