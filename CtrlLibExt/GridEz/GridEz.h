...
public:
	...
	{
		return InsertHeightedRow(index, DEFAULT_ROW_HEIGHT, std::forward<Types>(args)...);
	}
	...
	{
		return InsertRow(GetCount() - GetFixedBottomRowCount(), std::forward<Types>(args)...);
	}

	template <typename... Types>
	Row& AddHeightedNonfixedRow(int height, Types&&... args)
	{
		return InsertHeightedRow(GetCount() - GetFixedBottomRowCount(), height,
		                         std::forward<Types>(args)...);
	}

	bool IsColResizable(int col_index) const { return cols[col_index].IsResizable(); }
	...
	Value Get(int col) const { return Get0(currow, col); }
	...
	GridEz& FitColumns(bool fit = true)
	{
		fitcolumns = fit;
		return *this;
	}
	...
	Self&	HighlightCurRow(bool value){ highlightcurrow = value; return *this; }
	bool	HighligthCurRow()const{ return highlightcurrow; }
	...
	{
		return const_cast<const Row&>(const_cast<GridEz&>(*this)[i]);
	}

	int GetRowCount() const { return rows.size(); }

	int GetCount() const { return GetRowCount(); }

	int GetColCount() const { return cols.size(); }

	int GetRowHeight(int row_index)
	{
		ASSERT(row_index >= 0 && row_index < rows.GetCount());
		return rows[row_index].Height() == DEFAULT_ROW_HEIGHT ? defaultrowheight
	...
	int GetColWidth(int col_index)
	{
		...
		return cols[col_index].Width() == DEFAULT_COL_WIDTH ? defaultcolwidth
		                                                    : cols[col_index].Width();
	}

	int GetColFullWidth(int col_index)
	{
		ASSERT(col_index >= 0 && col_index < rows.GetCount());
		return cols[col_index].FullWidth() == DEFAULT_COL_WIDTH ? defaultcolwidth
		                                                        : cols[col_index].FullWidth();
	}

	int GetIndexOfFirstNonfixedRow() const { return GetFixedTopRowCount(); }

	int GetIndexOfLastNonfixedRow() const { return GetCount() - GetFixedBottomRowCount() - 1; }

	int GetNonfixedRowCount() const { return GetRowCount() - GetFixedRowCount(); }

	bool IsFixedRowWantCursor() const { return fixedrowwantcursor; }

	bool IsFixedColWantCursor() const { return fixedcolwantcursor; }
	...
	bool IsFixedRow(int r) const { return IsFixedTopRow(r) || IsFixedBtmRow(r); }

	bool IsFixedCol(int c) const { return IsFixedLeftCol(c) || IsFixedRightCol(c); }

	bool IsRowInA(int r) const { return IsFixedTopRow(r); }

	bool IsRowInE(int r) const { return !IsFixedTopRow(r) && !IsFixedBtmRow(r); }

	bool IsRowInI(int r) const { return IsFixedBtmRow(r); }

	int GetFixedLeftColCount() const { return fixedleftcols; }

	int GetFixedRightColCount() const { return fixedrightcols; }

	int GetFixedTopRowCount() const { return fixedtoprows; }

	int GetFixedBottomRowCount() const { return fixedbtmrows; }
	...
	int GetRowIndexAt(int y) const { return cached_data.map.GetRowIndexAt(y); }

	int GridToFPx(int x) const { return cached_data.map.col.GridToFullPicture(x); }

	int GridToFPy(int y) const { return cached_data.map.row.GridToFullPicture(y); }

	Point GridToFP(Point p) const { return {GridToFPx(p.x), GridToFPy(p.y)}; }
	...
	bool IsCurRow(int row) const { return row == currow; }

	int GetCurRowNo() const { return currow; }
	...
	bool IsCursor() const { return currow >= 0; }

	int GetCursor() const { return GetCurRowNo(); }
...
private:
	...
	bool DetermineCellPosition(int screenr, int screenc, int region_index, int r,
	                           CellPainter& di);
	void DetermineCellFormat(const CellFormat& rowf, const CellFormat& colf,
...
protected:
	...
	// static	int32 resizing_rc; // index of the col/row in the full picture that's be resized
	// or
