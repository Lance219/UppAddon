...
int Connection::Result::Status(){
    return (int)PQresultStatus(_res);
}

int Connection::Result::FieldCount()const
{
...
int Connection::Result::RowsAffected()
{
	return atoi(PQcmdTuples(_res));
}

int Connection::Result::RecordCount()const{
...
bool Connection::TableHasColumn(const char* tblname, const char * colname)
{
	String sql=String().Cat()
		...
		<<sql_quote(tblname)
		<<"AND column_name="<<sql_quote(colname);
...
void Connection::TestPG()
{
	DUMP(PQlibVersion());
	ExecSQL("DELETE FROM config WHERE key IN ('first name', 'last name')");
	Result r=Exec("INSERT INTO config(key, value) VALUES('first name','lance') RETURNING id");
	DUMP((int)PQresultStatus(r.GetRes()));
	r=Exec("INSERT INTO config(key, value) VALUES('last name','zhang')");
	DUMP((int)PQresultStatus(r.GetRes()));
	r=Exec("SELECT * FROM config");
	DUMP((int)PQresultStatus(r.GetRes()));
	r=Exec("SELECT * FROM config WHERE false");
	DUMP((int)PQresultStatus(r.GetRes()));
}
...
String Connection::ExportTableDef(String tbl)
{
	...
	auto t=Open(String().Cat()<<"SELECT column_name FROM information_schema.key_column_usage WHERE table_name="
		<<sql_quote(tbl)<<" AND position_in_unique_constraint IS NULL ORDER BY ordinal_position");
...
WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_name=)" << sql_quote(tbl);
	...
	for(int r=0; r<this->M(); ++r)
	{
		...
		for(int c=0; c<(int)rs.GetRecordDef().GetFieldCount(); ++c)
		{
			...
			if(1==PQfformat(_res, c)/*binary*/)
			{
				...
				//case 703: //"reltime"
				case 1114: // TIMESTAMP
					rs(c).As<Time>().Set(ntohll(*(int64*)s)/1000000+(2000LL*365+485)*24*3600);
					break;
				default:
...
const char * Connection::ErrorMessage()const
{
	return PQerrorMessage(_conn);
}

Oid Connection::lo_import(const char * filename)
{
	return ::lo_import(_conn, filename);
}
...
int Connection::lo_open (Oid oidBlob)
{
	return ::lo_open(_conn, oidBlob, INV_READ);
}

int Connection::lo_close ( int fd)
{
	return ::lo_close (_conn, fd);
}
...
const char * Connection::DatabaseName()const
{ 	return PQdb(_conn); }
const char * Connection::User()const
{ return PQuser(_conn); }
const char * Connection::Password()const
{ return PQpass(_conn); }
const char * Connection::Host()const
{ return PQhost(_conn); }
const char * Connection::Port()const
{ return PQport(_conn); }
	// asynchronous processing
bool Connection::SendQuery(const char * command)
{
	return PQsendQuery(_conn, command)==1;
}

Connection::Result Connection::GetResult()
{	
	return Result(PQgetResult(_conn));
}

bool Connection::ConsumeInput()
{
	return PQconsumeInput(_conn)==1;
}
...
again:
	if(rs(pkey_index).IsNull())// new record
	{
		...
		sql<<")VALUES("<<s<<") RETURNING "
			<<rdef.GetFieldDef(pkey_index).GetFieldName();
		try{
			auto t=Open(sql);
