...
public:
	...
	RecordSet& operator[](int index) { return recordSets[index]; }

	const RecordSet& operator[](int index) const { return recordSets[index]; }

	RecordSet& operator[](const char* name) { return (*this)[IndexOf(name)]; }

	const RecordSet& operator[](const char* name) const { return (*this)[IndexOf(name)]; }

	unsigned GetCount() const { return recordSets.GetCount(); }

	void Reserve(int num_rset) { recordSets.Reserve(num_rset); }
	...
	bool Contains(const char* name) const { return IndexOf(name) != -1; }

	void Clear() { recordSets.Clear(); }
