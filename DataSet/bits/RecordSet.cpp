...
{
	return fldDef->tid == FT_BIT ?
		   ( GetBit ( base, fldDef->start ) ? ( void* ) 1 : ( void* ) 0 ) :
...
static void * SetToTime ( void * dest, int id, const void* data )
{
	...
	switch ( id )
	{
		...
		case FT_PCHAR:
			...
			if ( StrToTime ( t, ( const char* ) ( data ) ) == NULL )
				t = Null;
...
//		}
	...
	memmove ( const_cast<char*> ( GetRowBase ( i ) ), GetRowBase ( i + 1 ), ( recordCount - i - 1 ) *recordSize );
...
RecordSet& RecordSet::Erase ( int i, int j )
{
	...
	if ( j != recordCount - 1 )
		memmove ( const_cast<char*> ( GetRowBase ( i ) ), GetRowBase ( j + 1 ), ( recordCount - j - 1 ) *recordSize );
...
RecordSet& RecordSet::SetRecordDef ( const RecordDef& rdf )
{
	...
	{
		LOG ( "RecordSet::SetRecordDef()-Modifying the RecordDef for a populated RecordSet ignored!" );
	}
...
RecordSet& RecordSet::AppendRecordDef ( const RecordDef& rdf )
{
	if ( this->RecordCount() != 0 )
	{
		LOG ( "RecordSet::AppendRecordDef()-Modifying the RecordDef for a populated RecordSet ignored!" );
	}
