...
inline void ClearBit(void* start, size_t offset) { UnsetBit(start, offset); }
...
public:
	...
	size_t Length() const { return p ? p->len : 0; }

	char* Bytes() { return p->data; }

	const char* Bytes() const { return p->data; }
...
enum FieldTypeId : unsigned char {
...
template <FieldTypeId>
struct Id2Type;

#define Id2TypeEntry(Id, Type)                                                                 \
	template <>                                                                                \
	struct Id2Type<Id> {                                                                       \
		typedef Type type;                                                                     \
	}
...
#ifdef _DEBUG
#define L_ENTRY(name, id, Type)                                                                \
	{                                                                                          \
		id, name, Type2Id<Id2Type<id>::type>::size, Type2Id<Id2Type<id>::type>::align          \
	}
#else
#define L_ENTRY(name, id, Type)                                                                \
	{                                                                                          \
		name, Type2Id<Id2Type<id>::type>::size, Type2Id<Id2Type<id>::type>::align              \
	}
#endif
...
inline static const TypeInfo basic_types[] = {
	...
	//	FT_BLOB = 15,
	//	FT_VALUE = 16, //u++ value
	L_ENTRY("bit", FT_BIT, Bool),         L_ENTRY("bool", FT_BOOL, Bool),
	L_ENTRY("int8", FT_INT8, Int8),       L_ENTRY("int16", FT_INT16, Int16),
	L_ENTRY("int32", FT_INT32, Int32),    L_ENTRY("int64", FT_INT64, Int64),
	L_ENTRY("uint8", FT_UINT8, UInt8),    L_ENTRY("uint16", FT_UINT16, UInt16),
	L_ENTRY("uint32", FT_UINT32, UInt32), L_ENTRY("uint64", FT_UINT64, UInt64),
	L_ENTRY("float", FT_FLOAT, Float),    L_ENTRY("double", FT_DOUBLE, Double),
	L_ENTRY("string", FT_STRING, String), L_ENTRY("date", FT_DATE, Date),
	L_ENTRY("time", FT_TIME, Time),       L_ENTRY("blob", FT_BLOB, Blob),
